...
export class ReportsPage implements OnInit, AfterViewInit {
  ...
  // Summary stats - ALL calculated from procurement API
  summaryStats = {
    totalValue: 0,
    avgValue: 0,
    completionRate: 0,
  ...
  loadAllProcurements(): void {
    ...
    this.procurementService.getAll({ limit: 1000 }).subscribe({
      next: (response: any) => {
        ...
        // Apply filters and update reports
    ...
    // Load campus distribution from dashboard API (this one doesn't need filtering)
  ...
  applyFiltersAndUpdate(): void {
    ...
    // Filter by date range (using bid_closing or created_at)
    if (this.reportFilters.date_from) {
      ...
      filteredProcurements = filteredProcurements.filter(p => {
        const procDate = p.bid_closing ? new Date(p.bid_closing) : new Date(p.created_at);
    ...
    console.log('Filtered procurements:', filteredProcurements.length);
    
    // Calculate statistics from filtered data
    this.calculateStatisticsFromData(filteredProcurements);
    
    // Update status distribution for chart
    this.updateStatusDistribution(filteredProcurements);
    
    // Update trend data based on filtered procurements
