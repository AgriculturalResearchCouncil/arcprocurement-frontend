...
export class ReportsPage implements OnInit, AfterViewInit {
  ...
  loadReports(): void {
    ...
    this.dashboardService.getStatusDistribution().subscribe({
      ...
      error: (error: any) => console.error('Error loading status distribution:', error)
    });
    ...
    this.dashboardService.getByCampus().subscribe({
      ...
      error: (error: any) => console.error('Error loading campus data:', error)
    });
    ...
    this.dashboardService.getTrends().subscribe({
      ...
      error: (error: any) => console.error('Error loading trends:', error)
    });
  }
   ...
   * Fetches actual procurement data to calculate totals
   */
    ...
    this.dashboardService.getSummary().subscribe({
      next: (summary: any) => {
        ...
        if (summary.total_procurements > 0) {
          // Calculate how many procurements were completed on time
          // This would come from your backend - for now using completion rate as base
          this.summaryStats.onTimeRate = this.summaryStats.completionRate;
        }
      },
      error: (error) => console.error('Error calculating summary stats:', error)
    });
  }
    ...
    this.campusChart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: '#005D28',  // ARC Green
          borderRadius: 4
        }]
      },
      options: {
  ...
  /**
   * Format currency (would need actual data from backend)
