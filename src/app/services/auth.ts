  ...
  login(username: string, password: string): Observable<LoginResponse> {
    ...
    return this.http.post<any>(url, body).pipe(
      map((response: any) => {
        ...
        // Backend returns { token, user } without a 'success' field
        const isSuccess = response.token && response.user;
        
        if (!isSuccess) {
          const errorMsg = response.message || response.error || 'Authentication failed';
          throw new AuthError(
