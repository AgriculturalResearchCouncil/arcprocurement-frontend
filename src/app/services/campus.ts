...
import { Campus } from '../models/campus.model';  // Import from models instead of defining locally
...
export class CampusService {
    ...
    return this.http.get<any[]>(`${this.dashboardUrl}/by-campus`).pipe(
      map(campusData => {
        ...
        return campusData.map(item => ({
          id: item.campus_id || item.id,
          name: item.campus_name || item.name,
          campus_name: item.campus_name || item.name,
  ...
  getById(id: number): Observable<Campus> {
    return this.http.get<any[]>(`${this.dashboardUrl}/by-campus`).pipe(
      map(campusData => {
        const campus = campusData.find(c => (c.campus_id || c.id) === id);
        if (campus) {
          return {
            id: campus.campus_id || campus.id,
            name: campus.campus_name || campus.name,
            campus_name: campus.campus_name || campus.name,
            location: campus.location || '',
            created_at: campus.created_at || new Date().toISOString()
          } as Campus;
        }
